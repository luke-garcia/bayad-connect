...
public final class Globe extends AppCompatActivity {

    DecimalFormat decimalFormat = new DecimalFormat("##,##0.00");
    Button button_submit;
    EditText field_accountno, field_amountdue, field_passon, field_accountname, field_telephoneno, field_duedate;
    TextView text_Total, text_wallet, text_duedate;
    ImageView image;
    String apireturn = null;
    String sServiceCode, sBillerCode, sWallet, sTpaid,  sOtherInfo,sAccountno,sAmountdue,sPasson,sAmounttopay,sBillName;
    int iImage;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        field_telephoneno = (EditText) findViewById(R.id.field_telephoneno);
        ...
        field_accountname.addTextChangedListener(checkAlphaChar);
        ...
        if (sServiceCode.equals("INNOV")) {
            //show duedate
            text_duedate.setVisibility(View.VISIBLE);
            field_duedate.setVisibility(View.VISIBLE);
            field_duedate.setText(new Functions(this).CommonDate());
            this.setTitle("Innove");
        }
    ...
    private DatePickerDialog.OnDateSetListener dateListener = new DatePickerDialog.OnDateSetListener() {
        ...
        public void onDateSet(DatePicker view, int year, int month, int dayOfMonth) {
            String today = String.valueOf(month + 1) + "/" + String.valueOf(dayOfMonth) + "/" + String.valueOf(year);
            field_duedate.setText(today);
        }
    };
    ...
    public void validate() {
        ...
        if(field_accountno.getText().toString().isEmpty()){
            ...
            if(sServiceCode.equals("GLOBE")){
                if (field_accountno.getText().toString().length() == 8 || field_accountno.getText().toString().length() == 10) {
                    validateTheOtherInfo();
                }else {
                    showMessage(getString(R.string.error_globedigit));
                }
            }else{
                if (field_accountno.getText().toString().length() == 9) {
                    validateTheOtherInfo();
                }else {
                    showMessage(getString(R.string.error_acc9digits));
                }
            }
        }
    ...
    public void validateTheOtherInfo(){
    ...
    public void connectToAPI_Validation(){
        ...
        Globe globe = this;
        new Functions(this).connectToAPI_Validation(sTpaid, sWallet, sServiceCode, sBillerCode, sAccountno, sAmountdue, sPasson, sAmounttopay, sOtherInfo, sBillName,globe);
    ...
    public String OtherInfo(){
        if(sServiceCode.equals("GLOBE")) {
            sOtherInfo = getString(R.string.ts_accname) + field_accountname.getText().toString() + getString(R.string.te_accname) +
                    getString(R.string.ts_telephoneno) + field_telephoneno.getText().toString() + getString(R.string.te_telephoneno);
        }else{
            sOtherInfo = getString(R.string.ts_accname) + field_accountname.getText().toString() + getString(R.string.te_accname) +
                    getString(R.string.ts_telephoneno) + field_telephoneno.getText().toString() + getString(R.string.te_telephoneno)+
