...
public final class SkyCable extends AppCompatActivity {

    DecimalFormat decimalFormat = new DecimalFormat("##,##0.00");
    Button button_submit;
    EditText field_accountno,field_amountdue,field_passon,field_duedate,field_billdate;
    TextView text_Total,text_wallet,text_srn;
    ImageView image;
    String sServiceCode,sBillerCode,sWallet,sTpaid,apireturn,sSkycableCode, sAccountno, sAmountdue, sPasson, sAmounttopay, sOtherInfo, sBillName;
    int iImage;
    Double amountdue,passon,total;
    ProgressDialog progressDialog;
    Spinner spinner_skycable;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        spinner_skycable = (Spinner) findViewById(R.id.spinner_skycable);
        ...
        spinner_skycable.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                sSkycableCode = spinner_skycable.getSelectedItem().toString();
                switch (sSkycableCode){
                    case "Destiny Cable":
                        sSkycableCode = "0";
                        break;
            ...
            if(field_amountdue.getText().toString().isEmpty()){
                ...
                if(Double.parseDouble(field_amountdue.getText().toString()) <= 35000.00){
                    ...
                    if(field_duedate.getText().toString().isEmpty()){
                        ...
                        if(field_billdate.getText().toString().isEmpty()){
                            showMessage(getString(R.string.error_billdate));
                        }else if(new Functions(this).checkDateFormat(field_billdate.getText().toString())){

                            connectToAPI_Validation();
                        }else{
                            showMessage(getString(R.string.error_billdate_format));
                        }
                    }else{
    ...
    public void connectToAPI_Validation(){
        ...
        SkyCable skyCable = this;
        new Functions(this).connectToAPI_Validation(sTpaid, sWallet, sServiceCode, sBillerCode, sAccountno, sAmountdue, sPasson, sAmounttopay, sOtherInfo, sBillName,skyCable);
    }


    public String OtherInfo(){
        sOtherInfo = getString(R.string.ts_sertype)+sSkycableCode+getString(R.string.te_sertype)+
