...
public final class CebuAir extends AppCompatActivity {

    DecimalFormat decimalFormat = new DecimalFormat("##,##0.00");
    Button button_submit;
    EditText field_accountno,field_amountdue,field_passon,field_payorname;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_cebu_air);
        ...
        field_payorname = (EditText) findViewById(R.id.field_payorname);
        ...
        field_payorname.addTextChangedListener(checkPayorName);
    ...
    private final TextWatcher checkPayorName = new TextWatcher() {
        @Override
        public void beforeTextChanged(CharSequence s, int start, int count, int after) {
        }
        ...
        public void afterTextChanged(Editable s) {
            ...
            if (s.toString().length() > 0 && !s.toString().matches(namepattern)) {
                int length = field_payorname.getText().length();
                field_payorname.getText().delete(length -1, length);
            }
    ...
    public void validate() {
        ...
        if(field_accountno.getText().toString().isEmpty()){
            ...
            showMessage(getString(R.string.error_relocno));

        }else if(field_accountno.getText().toString().length() == 12){
            ...
            if(field_amountdue.getText().toString().isEmpty()){
                ...
                if(Double.parseDouble(field_amountdue.getText().toString()) <= 35000.00){
                    if(field_payorname.getText().toString().isEmpty()){
                        showMessage(getString(R.string.error_payorname));
                    }else{
            ...
            showMessage(getString(R.string.error_reloc12digits));
        }
    ...
    public void connectToAPI_Validation(){
        ...
        CebuAir cebuAir = this;
        new Functions(this).connectToAPI_Validation(sTpaid, sWallet, sServiceCode, sBillerCode, sAccountno, sAmountdue, sPasson, sAmounttopay, sOtherInfo, sBillName,cebuAir);

    }


    public String OtherInfo(){
        sOtherInfo = getString(R.string.ts_payorname)+field_payorname.getText().toString()+getString(R.string.te_payorname);
