...
public final class HomeCredit extends AppCompatActivity {

    DecimalFormat decimalFormat = new DecimalFormat("##,##0.00");
    Button button_submit;
    EditText field_referenceno,field_amountdue,field_passon,field_name,field_phoneno;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        field_referenceno = (EditText) findViewById(R.id.field_referenceno);
        ...
        field_phoneno = (EditText) findViewById(R.id.field_phoneno);
        ...
        field_name.addTextChangedListener(checkAlphaChar);
    ...
    private final TextWatcher checkAlphaChar = new TextWatcher() {
        @Override
        public void beforeTextChanged(CharSequence s, int start, int count, int after) {
        }
        ...
        public void afterTextChanged(Editable s) {
            ...
            if (s.toString().length() > 0&& !s.toString().matches(namepattern)) {
                int length = field_name.getText().length();
                field_name.getText().delete(length -1, length);
            }
    ...
    public void validate() {
        ...
        if(field_referenceno.getText().toString().isEmpty()){

            showMessage(getString(R.string.error_refnumber));

        }else if(field_referenceno.getText().toString().length() == 10){

            if(field_name.getText().toString().isEmpty()){
                showMessage(getString(R.string.error_name));
            }else {
                if(field_amountdue.getText().toString().isEmpty()){
                    ...
                    if(Double.parseDouble(field_amountdue.getText().toString()) <= 35000.00){
                        ...
                        if(field_phoneno.getText().toString().isEmpty()){
                            connectToAPI_Validation();
                        }else if(field_phoneno.getText().toString().length()>=6 && field_phoneno.getText().toString().length()<=12){
                            connectToAPI_Validation();
                        }else{
                            showMessage(getString(R.string.error_phonenolength612));
                        }
            ...
            showMessage(getString(R.string.error_multipayref));
        }
    ...
    public void connectToAPI_Validation(){
        sAccountno = field_referenceno.getText().toString();
        ...
        HomeCredit homeCredit = this;
        new Functions(this).connectToAPI_Validation(sTpaid, sWallet, sServiceCode, sBillerCode, sAccountno, sAmountdue, sPasson, sAmounttopay, sOtherInfo, sBillName,homeCredit);
    }


    public String OtherInfo(){
        sOtherInfo = getString(R.string.ts_name)+field_name.getText().toString()+getString(R.string.te_name)+
                getString(R.string.ts_phoneno)+field_phoneno.getText().toString()+getString(R.string.te_phoneno);
        return sOtherInfo;
    }
