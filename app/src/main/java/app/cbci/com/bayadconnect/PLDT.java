...
public final class PLDT extends AppCompatActivity {

    DecimalFormat decimalFormat = new DecimalFormat("##,##0.00");
    Button button_submit;
    EditText field_accountno,field_amountdue,field_passon,field_phoneno;
    TextView text_Total,text_wallet,text_duedate;
    ImageView image;
    String sServiceCode,sBillerCode,sWallet,sTpaid,apireturn,sPLDTService,sAccountno, sAmountdue, sPasson, sAmounttopay, sOtherInfo, sBillName;
    int iImage;
    Double amountdue,passon,total;
    ProgressDialog progressDialog;
    Spinner spinner_pldt;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        spinner_pldt = (Spinner) findViewById(R.id.spinner_pldt);
        ...
        spinner_pldt.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                sPLDTService = spinner_pldt.getSelectedItem().toString();
            ...
            if(field_amountdue.getText().toString().isEmpty()){
                ...
                if(Double.parseDouble(field_amountdue.getText().toString()) <= 35000.00){
                    if(field_phoneno.getText().toString().isEmpty()){
                        showMessage(getString(R.string.error_phoneno));
                    }else if(field_phoneno.getText().toString().length() == 10){
                        connectToAPI_Validation();
                    }else{
                        showMessage(getString(R.string.error_pldtphone));
                    }
    ...
    public void connectToAPI_Validation(){
        ...
        PLDT pldt = this;
        new Functions(this).connectToAPI_Validation(sTpaid, sWallet, sServiceCode, sBillerCode, sAccountno, sAmountdue, sPasson, sAmounttopay, sOtherInfo, sBillName,pldt);
    }


    public String OtherInfo(){
        sOtherInfo = getString(R.string.ts_phonenumber)+field_phoneno.getText().toString()+getString(R.string.te_phonenumber)+
                getString(R.string.ts_service)+sPLDTService+getString(R.string.te_service);
        return sOtherInfo;
    }
